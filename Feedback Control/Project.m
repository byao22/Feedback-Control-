...
% Kambiz Shoarinejad, Feb 26, 2018 
...
P_elec = tf([5],[2E-4, 4.2]) %electrical plant transfer function: Vs/Vc

% Designing the current controller
R2 = [5000] %value of resistor R2
R3 = [121288] %value of resistor R3
C1 = [1.374E-10] %value of capacitor C1
C2 = [1.3908E-12] %value of capacitor C2

C_elec = tf([R3*C1 , 1],[R2*R3*C1*C2, R2*C2+R2*C1, 0]); %electrical controller transfer function -Vc/Vs
            %specify the transfer function in terms of the resistor and
            %capacitor values given above
H_vsic = [1/(5*Rs)]             % transfer function: Ic/Vs
...
Gp = tfest(data, 10)
%Gp = [tf([10^2], [1, 100.2, 10^6],'InputDelay', 1.5E-5)]     % fitted mechanical plant Gp_fit=X/F
bode(Gp)
